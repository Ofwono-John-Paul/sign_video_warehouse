...
class _AdminDashboardState extends State<AdminDashboard>
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: _loading
          ...
          : TabBarView(
              controller: _tabs,
              children: [_overviewTab(cs), _mapTab(cs), _schoolsTab(cs), _videosTab(cs)],
            ),
    );
  }
      ...
      child: ListView(
        ...
        children: [
          ...
          GridView.count(
            ...
            children: [
              _kpi('Total Videos', total.toString(), Icons.video_library, cs.primary),
              _kpi('Approved',     approved.toString(), Icons.verified,   Colors.green),
              _kpi('Pending',      pending.toString(), Icons.hourglass_top, Colors.orange),
              _kpi('Schools',      schools.toString(), Icons.school,      const Color(0xFF6A1B9A)),
            ],
          ),
                ...
                titlesData: FlTitlesData(
                  bottomTitles: AxisTitles(sideTitles: SideTitles(
                    ...
                    getTitlesWidget: (v, _) {
                      ...
                      final region = (_regions[idx]['region'] as String?) ?? '';
                      return Transform.rotate(angle: -0.5,
                          child: Text(region, style: const TextStyle(fontSize: 9)));
                    },
                  )),
              ...
              child: PieChart(PieChartData(
                sections: cats.asMap().entries.map((e) {
                  final colors = [
                    ...
                    const Color(0xFF00838F), const Color(0xFFF57F17),
                  ];
  ...
  Widget _schoolsTab(ColorScheme cs) {
    if (_schools.isEmpty) return const Center(child: Text('No schools registered yet.'));
    return RefreshIndicator(
      onRefresh: _load,
        ...
        itemBuilder: (_, i) {
          ...
          return ListTile(
            leading: CircleAvatar(
              backgroundColor: (_regionColors[region] ?? cs.primary).withOpacity(0.15),
              child: Icon(Icons.school, color: _regionColors[region] ?? cs.primary),
            ),
            title: Text(s['name'] ?? '', maxLines: 1, overflow: TextOverflow.ellipsis),
            subtitle: Text('${s['district']} · ${s['region']}  |  ${s['uploads'] ?? 0} uploads'),
            trailing: Column(
              mainAxisAlignment: MainAxisAlignment.center,
              crossAxisAlignment: CrossAxisAlignment.end,
              children: [
                Text('${s['deaf_students'] ?? 0} deaf',
                    style: const TextStyle(fontSize: 11, fontWeight: FontWeight.bold)),
                Text(s['school_type'] ?? '', style: const TextStyle(fontSize: 10, color: Colors.grey)),
              ],
            ),
          );
        },
            ...
            trailing: status == 'pending'
                ? Row(mainAxisSize: MainAxisSize.min, children: [
                    IconButton(
                      ...
                      onPressed: () => _verify(v['video_id'] as int, 'approved'),
                    ),
                    IconButton(
                      icon: const Icon(Icons.cancel, color: Colors.red),
                      onPressed: () => _verify(v['video_id'] as int, 'rejected'),
                    ),
                  ])
                : Chip(
